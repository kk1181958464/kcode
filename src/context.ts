...
export const estimateMessageTokens = (items: ChatMessage[]) => Math.ceil(items.reduce((total, item) =>
  total + item.content.length + (item.images?.reduce((size, image) => size + Math.min(image.size, 750_000), 0) ?? 0) / 750,
0) / 3);

export function compactConversation(task: CompactableContext, contextWindow: number, force = false) {
  ...
  for (let index = task.messages.length - 1; index >= alreadyCompacted; index--) {
  ...
  compactUntil = Math.min(compactUntil, Math.max(alreadyCompacted, task.messages.length - 2));
  if (force && compactUntil <= alreadyCompacted && task.messages.length - alreadyCompacted > 2) compactUntil = task.messages.length - 2;
  ...
  const goals: string[] = [], decisions: string[] = [], results: string[] = [], errors: string[] = [], pending: string[] = [];
  for (const message of older) {
    const role = message.role === "user" ? "用户" : `模型${message.model ? `(${message.model})` : ""}`;
    const text = message.content.replace(/\s+/g, " ").trim();
    const semantics = message.images?.map((image) => task.imageSemantics?.[image.id]).filter(Boolean).join("；");
    const imageNote = message.images?.length ? ` [图片语义：${semantics || `${message.images.length} 张图片，尚无描述`}]` : "";
    const line = `- ${role}: ${text.slice(0, 700)}${text.length > 700 ? "…" : ""}${imageNote}`;
    if (/失败|错误|报错|异常|error|failed/i.test(text)) errors.push(line);
    else if (/完成|通过|已修改|已添加|已修复|构建|测试/i.test(text)) results.push(line);
  ...
  for (const activity of task.activities.filter((item) => item.completedAt))
    latestActivities.set(`${activity.tool}:${activity.path ?? activity.command ?? activity.title}`, activity);
  const dedupedActivities = [...latestActivities.values()].slice(-24);
  const activityLines = dedupedActivities.map((activity) => `- ${activity.title}${activity.path ? ` ${activity.path}` : ""}: ${activity.status}${activity.errorSummary ? `，${activity.errorSummary}` : ""}`);
